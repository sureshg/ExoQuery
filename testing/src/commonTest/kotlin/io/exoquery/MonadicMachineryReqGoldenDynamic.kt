...
object MonadicMachineryReqGoldenDynamic: GoldenQueryFile {
  override val queries = mapOf<String, GoldenResult>(
    ...
    "capture.expression.use.(@Cap (Row)()->Table)/XR" to kt(
      "select { val p = from(Table(Person)); val a = from({ this -> Table(Address).join { a -> this.id == a.ownerId }.toExpr }.apply(p).toQuery); val r = from({ this -> Table(Robot).join { r -> this.id == r.ownerId }.toExpr }.apply(p).toQuery) }"
    ),
    "capture.expression.use.(@Cap (Row)()->Table)/SQL" to cr(
      "SELECT p.id, p.name, p.age, a.ownerId, a.street, a.city, r.ownerId, r.name, r.model FROM Person p INNER JOIN Address a ON p.id = a.ownerId INNER JOIN Robot r ON p.id = r.ownerId"
    ),
    "capture.(@Cap (Row)()->Table)/XR" to kt(
      "select { val p = from(Table(Person)); val a = from({ this -> Table(Address).join { a -> this.id == a.ownerId } }.toQuery.apply(p)); val r = from({ this -> Table(Robot).join { r -> this.id == r.ownerId } }.toQuery.apply(p)) }"
    ),
