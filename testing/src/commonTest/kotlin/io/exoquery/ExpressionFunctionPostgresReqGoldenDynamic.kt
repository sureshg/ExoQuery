...
object ExpressionFunctionPostgresReqGoldenDynamic: GoldenQueryFile {
  override val queries = mapOf<String, GoldenResult>(
    "String/toInt" to cr(
      "SELECT p.name::INTEGER AS value FROM Person p"
    ),
    "String/toLong" to cr(
      "SELECT p.name::BIGINT AS value FROM Person p"
    ),
    "String/toBoolean" to cr(
      "SELECT p.name::BOOLEAN AS value FROM Person p"
    ),
    ...
    "Int/toDouble" to cr(
      "SELECT p.age:DOUBLE PRECISION AS value FROM Person p"
    ),
    "Int/toFloat" to cr(
      "SELECT p.age:REAL AS value FROM Person p"
    ),
    "Long/toInt" to cr(
      "SELECT p.age AS value FROM Person p"
    ),
