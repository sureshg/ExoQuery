...
object QueryDistinctReqGoldenDynamic: GoldenQueryFile {
  override val queries = mapOf<String, GoldenResult>(
    ...
    "distinct simple/(selectClause + aggregation).distinct/XR" to kt(
      "select { val p = from(Table(Person)); val a = join(Table(Address)) { p.id == a.ownerId }; groupBy(p.name) }.distinct"
    ),
    ...
    "distinct simple/table.join(table.distinct)/XR" to kt(
      "select { val p = from(Table(Person)); val a = join(Table(Address).distinct) { p.id == a.ownerId } }"
    ),
    ...
    "distinct simple/from.distinct, sort.join/XR" to kt(
      "select { val p = from(Table(Person).distinct); val a = join(Table(Address).sortBy(Asc) { it -> it.city }) { p.id == a.ownerId } }"
    ),
