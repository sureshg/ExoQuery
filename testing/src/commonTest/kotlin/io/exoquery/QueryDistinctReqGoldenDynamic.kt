...
object QueryDistinctReqGoldenDynamic: GoldenQueryFile {
  override val queries = mapOf<String, String>(
    ...
    "distinct simple/(selectClause + aggregation).distinct/XR" to kt(
      "select { val p = from(Table(Person)); val a = join(Table(Address)) { p.id == it.ownerId }; groupBy(p.name) }.distinct"
    ),
    "distinct simple/(selectClause + aggregation).distinct/SQL" to cr(
      "SELECT DISTINCT p.name AS first, max(p.age) AS second FROM Person p INNER JOIN Address it ON p.id = it.ownerId GROUP BY p.name"
    ),
    "distinct simple/table.join(table.distinct)/XR" to kt(
      "select { val p = from(Table(Person)); val a = join(Table(Address).distinct) { p.id == it.ownerId } }"
    ),
    "distinct simple/table.join(table.distinct)/SQL" to cr(
      "SELECT p.id, p.name, p.age, it.ownerId, it.street, it.city FROM Person p INNER JOIN (SELECT DISTINCT x.ownerId, x.street, x.city FROM Address x) AS it ON p.id = it.ownerId"
    ),
    "distinct simple/from.distinct, from/XR" to kt(
      "select { val p = from(Table(Person).distinct); val a = from(Table(Address).filter { it -> it.ownerId == p.id }) }"
    ),
    ...
    "distinct simple/from.distinct, sort.join/XR" to kt(
      "select { val p = from(Table(Person).distinct); val a = join(Table(Address).sortBy(Asc) { it -> it.city }) { p.id == it.ownerId } }"
    ),
