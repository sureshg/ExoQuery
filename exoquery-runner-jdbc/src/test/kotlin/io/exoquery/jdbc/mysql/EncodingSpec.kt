...
class EncodingSpec: FreeSpec({
  ...
  "Encode/Decode Java Types" - {
    ...
    "insert" {
      ...
      }.buildFor.MySql().runOn(ctx)
      val res = sql { Table<JavaTestEntity>() }.buildFor.MySql().runOn(ctx).let { it.firstOrNull() ?: error("Expected one element list but got: ${it}") }
      verify(res, ent)
    }
    "insert - setParams" {
      sql {
  ...
  "Encode/Decode EncodingTestEntity" - {
    ...
    "insert - ctx" {
      ...
      val res = sql { Table<EncodingTestEntity>() }.buildFor.MySql().runOn(ctx).let { it.firstOrNull() ?: error("Expected one element list but got: ${it}") }
      verify(res, ent)
    }
    "insert - ctx" {
      sql {
