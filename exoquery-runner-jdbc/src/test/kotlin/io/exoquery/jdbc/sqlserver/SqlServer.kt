...
class EncodingSpec: FreeSpec({
  ...
  "Encode/Decode Java Types" - {
    ...
    "insert" {
      ...
      }.buildFor.SqlServer().runOn(ctx)
      val res = sql { Table<JavaTestEntity>() }.buildFor.SqlServer().runOn(ctx).let { it.firstOrNull() ?: error("Expected one element list but got: ${it}") }
      verify(res, ent)
    }
    "insert - setParams" {
      sql {
  ...
  "Encode/Decode EncodingTestEntity" - {
    ...
    "insert - ctx" {
      ...
      val res = sql { Table<EncodingTestEntity>() }.buildFor.SqlServer().runOn(ctx).let { it.firstOrNull() ?: error("Expected one element list but got: ${it}") }
      verify(res, ent)
    }
    "insert - ctx" {
      sql {
