...
class JsonOpSpec : FreeSpec({
  ...
  "one level nesting" - {
    ...
    "select string field" {
      val ret = sql { Table<JsonExample>().map { it.value.name } }.buildFor.Sqlite().runOn(ctx)
      ret shouldContainExactlyInAnyOrder listOf("Joe", "Jim")
    }
    "select int field" {
      val ret = sql { Table<JsonExample>().map { it.value.age } }.buildFor.Sqlite().runOn(ctx)
      ret shouldContainExactlyInAnyOrder listOf(123, 456)
    }
    "filter by int field" {
      val ret = sql { Table<JsonExample>().filter { it.value.age > 200 } }.buildFor.Sqlite().runOn(ctx)
  ...
  "two level nesting" - {
    ...
    "select nested string field" {
      val ret = sql { Table<JsonExample>().map { it.value.address.street } }.buildFor.Sqlite().runOn(ctx)
      ret shouldContainExactlyInAnyOrder listOf("Main St", "Second St")
    }
    "select nested int field" {
      val ret = sql { Table<JsonExample>().map { it.value.address.number } }.buildFor.Sqlite().runOn(ctx)
      ret shouldContainExactlyInAnyOrder listOf(123, 456)
    }
    "filter by nested int field" {
      val ret = sql { Table<JsonExample>().filter { it.value.address.number > 200 } }.buildFor.Sqlite().runOn(ctx)
      ret shouldContainExactlyInAnyOrder listOf(JsonExample(2, jim))
    }
  }
  ...
  "three level nesting" - {
    ...
    "select three level nested string field" {
      val ret = sql { Table<JsonExample>().map { it.value.address.country.name } }.buildFor.Sqlite().runOn(ctx)
      ret shouldContainExactlyInAnyOrder listOf("USA", "Canada")
    }
    "select three level nested string field 2" {
      val ret = sql { Table<JsonExample>().map { it.value.address.country.code } }.buildFor.Sqlite().runOn(ctx)
      ret shouldContainExactlyInAnyOrder listOf("US", "CA")
    }
    "filter by three level nested string field" {
      val ret = sql { Table<JsonExample>().filter { it.value.address.country.code == "CA" } }.buildFor.Sqlite().runOn(ctx)
      ret shouldContainExactlyInAnyOrder listOf(JsonExample(2, jim))
    }
  }
})
