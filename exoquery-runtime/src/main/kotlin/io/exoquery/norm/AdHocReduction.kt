...
class AdHocReduction {
  ...
  fun doMatch(q: Query): XR.Query? =
    on(q).match(
      ...
      case(Filter[Filter[Is(), Is()], Is()]).then { (a, b, c), d, e ->
        val er = BetaReduction(e, d to b)
        Filter(a, b, XR.BinaryOp(c, BooleanOperator.and, er))
      },
      // ---------------------------
      // flatMap.*
      ...
      case(FlatMap[Is(), Union[Is(), Is()]]).then { a, b, (c, d) ->
        Union(FlatMap(a, b, c), FlatMap(a, b, d))
      },
      ...
      case(FlatMap[Is(), UnionAll[Is(), Is()]]).then { a, b, (c, d) ->
        UnionAll(FlatMap(a, b, c), FlatMap(a, b, d))
      }
    )
}
