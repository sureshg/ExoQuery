...
abstract class GeneratorBase<Conn: AutoCloseable, Results> {
  ...
  abstract fun log(msg: String): Unit
  ...
  fun isCurrentVersion(versionFile: VersionFile): Boolean =
    (config.codeVersion as? CodeVersion.Fixed)?.let { it.version == versionFile.currentVersion } ?: run {
      log("[ExoQuery] Entity-Gen: ignoring existing version file, as code version is not fixed.")
      false
    }

  fun run(forceRegen: Boolean = false): Unit {
    val (isCurrentVersion, foundVersionFile) =
      if (forceRegen)
        (false to null) // isCurrentVersion = false, foundVersionFile = null
      else {
        // If forced-regeneration is disabled, get the version file and check to see it is CodeVersion.Floating or
        // if it is CodeVersion.Fixed and it has changed (from the CurrentVersion.kt file on disk in the generation path root)
        ...
        (isCurrentVersion to foundVersionFile)
      }
    if (!isCurrentVersion) {
      fileWriter.resetCodeRoot(config.rootPath)
      val deliverable = compute()
      deliverable.files.forEach { file ->
        log("[ExoQuery] Entity-Gen: Writing file: ${fileWriter.printPath(file)}")
        if (!config.dryRun)
          fileWriter.write(file)
      }
      versionFileWriter.writeVersionFileIfNeeded(config)
    } else {
      log("[ExoQuery] Entity-Gen: skipping code generation, current version ${foundVersionFile?.currentVersion ?: "<IMPOSSIBLE_STATE>"} is up to date.")
    }
  }
